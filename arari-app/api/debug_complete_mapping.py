...
for row in range(1, 51):
    ...
    if label or value or days:
        ...
        value_str = f"{value:,.0f}" if isinstance(value, (int, float)) and value else str(value) if value else ""
        days_str = f"{days}" if days else ""

        print(f"Row {row:2d}: Label='{label_str:30s}' | Value={value_str:>15s} | Days={days_str:>5s}")
